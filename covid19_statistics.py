def normalize_data(n_cases, n_people, scale):
    ...
    for idx, n in enumerate(n_cases):
        norm_cases.append(n_cases[idx]/n_people[idx] * scale) # TODO) Calculate the number of cases per its population
    return norm_cases
...
for idx, pop in enumerate(n_people):
    ratio = (pop/sum_people) * 100 # TODO) Calculate the ratio of each local population to the total population
...
for idx, pop in enumerate(n_covid):
    ratio = (pop/sum_covid) * 100 # TODO) Calculate the ratio of each local population to the total population
